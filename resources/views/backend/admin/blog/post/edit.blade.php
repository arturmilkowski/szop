        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('backend.admins.blog.posts.update', $item) }}" method="POST" enctype="multipart/form-data" novalidate>
                        @csrf
                        @method('patch')
                        <input type="hidden" name="slug">
                        <x-form-group>
                            <x-input-label for="title">Tytuł</x-input-label>
                            <x-text-input type="text" id="title" name="title" value="{{ old('title', $item->title) }}" placeholder="Pole obowiązkowe" maxlength="255" required autofocus />
                            <x-input-error :messages="$errors->get('title')" />
                        </x-form-group>
                        <x-form-group>
                            <x-input-label for="intro">Wstęp</x-input-label>
                            <x-textarea-input id="intro" name="intro" placeholder="Pole nieobowiązkowe">{{ old('intro', $item->intro) }}</x-textarea-input>
                            <x-input-error :messages="$errors->get('intro')" />
                        </x-form-group>
                        <x-form-group>
                            <x-input-label for="content">Treść</x-input-label>
                            <x-textarea-input id="content" name="content" placeholder="Pole nieobowiązkowe">{{ old('content', $item->content) }}</x-textarea-input>
                        ...
                        <x-form-group>
                            ...
                            <x-text-input type="text" id="site_description" name="site_description" value="{{ old('site_description', $item->site_description) }}" placeholder="Pole nieobowiązkowe" maxlength="255" />
                        ...
                        <x-form-group>
                            ...
                            <x-text-input type="text" id="site_keyword" name="site_keyword" value="{{ old('site_keyword', $item->site_keyword) }}" placeholder="Pole nieobowiązkowe" maxlength="255" />
                            <x-input-error :messages="$errors->get('site_keyword')" />
                        </x-form-group>
                        <x-form-group-form-check>
                                @if(old('approved', $item->approved))
                        ...
                        <x-form-group-form-check>
                            @if(old('comments_allowed', $item->comments_allowed))
                    ...
                    <form action="{{ route('backend.admins.blog.posts.destroy', $item) }}" method="POST">
                        @csrf
                        @method('delete')
                        <x-danger-button class="mt-1" :href="route('backend.admins.blog.posts.destroy', $item)" onclick="event.preventDefault(); this.closest('form').submit();">
                            Usuń
                        </x-danger-button>
                    </form>
                    <p>
