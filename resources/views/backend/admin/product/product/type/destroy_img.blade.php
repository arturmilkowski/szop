...
@section('title', 'usuwanie grafiki')

@section('content')
            @component('backend.components.header')
                czy na pewno chcesz usunąć grafikę?
            @endcomponent
            <div class="row">
                <div class="col-sm">
                    <form action="{{-- route('backend.admins.products.types.destroy_img', [$product->id, $type->id]) --}}" method="POST">
